...
function ChatList({ rooms, selectedRoom, setSelectedRoom }) {
    return (
        <div className="w-1/4 h-full bg-white shadow-lg flex flex-col overflow-y-auto">
            <div className="p-4 text-lg font-semibold border-b">Daftar Chat</div>

            {rooms.map((chat) => (
                <button
                    key={chat.room.id}
                    className={`cursor-pointer flex items-center p-3 w-full text-left ${selectedRoom?.room.id === chat.room.id ? "bg-gray-300" : "hover:bg-gray-200"
                        }`}
